 ...
 */
public class CollectorHandler extends AbstractActor {
    ...
    public CollectorHandler(CollectorModel model) {
        this.model = model;
    }
...
//        getContext().actorSelection("../../../../configs/*").tell(new ContainerStatusChangedMsg(), getSelf());
    ...
    private void preHandle(CollectorProtocal.Report report) {

        long start = System.currentTimeMillis();
        try {
            long time = report.time;
            monitor.newWriteTime("difTime", System.currentTimeMillis() - time, true);

            String bodyMsg = report.data;
            String ip = report.ip;
            String userAgent = report.userAgent;
            ...
            if (json != null && json.size() >= 3) {
                ...
                if (model.getStatus() == CollectorStatus.OFF) {
                    log.info("采集器已关闭，不允许收集数据");
                    getSender().tell(new CollectorProtocal.ReportResult(StatusCodes.OK, "report false, status is off"), getSelf());

                } else {
                    getSender().tell(new CollectorProtocal.ReportResult(StatusCodes.OK, "report success"), getSelf());
